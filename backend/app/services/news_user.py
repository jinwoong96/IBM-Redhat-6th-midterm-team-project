...
class NewsuserService:
    ...
    async def add_newsuser(login_id:str,db:AsyncSession): #새로운 날짜의 뉴스를 랜덤배정 후 기록
        ...
        for i in effects:#영향 카테고리 에서 반복문돌려서 카테고리마다 지정된 등락폭안에서 랜덤 설정
            # effect_flu=round(random.uniform(int(i.flu_min),int(i.flu_max)), 2)
        ...
        for stock in user_current_stock:# 그 안에있는 종목들을 가져와서
            ...
            for i in category_effect:#카테고리별 영향을 반복해서 돌린후
                if i['category_name'] == stock['category_name']:#지정된 뉴스에 나온 종목이름과 전체종목이름중 같은게 있으면
                    apply_flu = round(random.uniform(i['flu_min'], i['flu_max']), 2)
                    break
            ...
            next_max_price = max(next_start_price,next_end_price)*(1+random.uniform(0,0.03))#종가나 시가 중 높은금액에 살짝 더높은 최고가
            next_min_price = min(next_start_price,next_end_price)*(1-random.uniform(0,0.03))#종가나 시가 중 낮은금액에 살짝 더낮은 최저가
